        ...
        @foreach ( $movies as $movie )
            <div class="bg-gray-800 p-4 rounded-lg relative group">
                <a href="{{ route('movie.show', $loop->index) }}">
                    <img src="{{ $movie['image'] }}" alt="{{ $movie['title'] }}" srcset="" class="w-full rounded-md">
                    ...
                    <div class="absolute top-2 right-2 space-x-2 opacity-0 group-hover:opacity-100 transition">
                        ...
                        <a href="{{ route('movie.destroy', $loop->index) }}" onclick="event.preventDefault(); confirm('Are You Sure?'); document.getElementById('delete-form-{{ $loop->index }}').submit();" class=" bg-red-600 p-1 rounded hover:bg-red-500">
                            ❌
                        </a>
